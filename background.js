...
chrome.downloads.onCreated.addListener(function (downloadItem) {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
        ...
        chrome.tabs.sendMessage(tabs[0].id, { data: downloadItem, type: "download-data" });
    });
...
chrome.downloads.onDeterminingFilename.addListener(function (item, suggest) {
    ...
    if (item.state === "in_progress" && item.referrer === "https://ads.google.com/"
    ) {
...
chrome.runtime?.onMessage?.addListener(async function (message, sender, sendResponse) {
    ...
    if (message.type === "download-final") {
        ...
        const res = await ajaxCall('POST', `api/keyword/save-keyword/${message.data.data.id}`, {
            data: message.data
        })
